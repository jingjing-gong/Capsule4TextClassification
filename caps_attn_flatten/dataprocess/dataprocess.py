...
parser.add_argument('--base-wd-freq', action='store', dest='base_wd_freq', default=3)
...
def extract(fn):
    ...
    with open(fn, 'r') as fd:
        for line in fd:
            ...
            doc2snt = doc.strip().split('<sssss>')
            doc2snt2wd = [snt.strip().split(' ') for snt in doc2snt]
            label_collect.append(label)
            doc_tok_collect.append(doc2snt2wd)
    return label_collect, doc_tok_collect
...
def constructLabel_dict(labels, savepath):
    ...
    if args.label2id is None:
        label2id = dict(list(zip(label_freq.keys(), [int(o)-1 for o in label_freq.keys()])))
    else:
